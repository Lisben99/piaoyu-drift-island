...
当前生产提交：`3e34aa40712d787dea5c5d478cda4ecfc31b48ec`（短提交 `3e34aa4`）
...
- 主分支：`main`
- 兼容部署分支：`master`
- `origin/main` 与 `origin/master` 均指向 `3e34aa4`
...
原“社区”已改名为“共鸣”，页面标题为“共鸣广场”。用户端采用紫粉色“梦境灵魂”视觉，管理后台不换肤。
...
- 推荐采用可解释规则：兴趣匹配 35、互动亲和 25、新鲜度 25、互动质量 15。
- 推荐/最新/关注流排除已删除内容。
...
- `index.html` 新增统一 `--dream-*` CSS 变量。
- 用户端卡片、按钮、输入框、弹窗、顶部/底部导航使用紫粉主题和浅玻璃表面。
- 保留危险操作的珊瑚红和等级金色。
...
$html.Contains('--dream-primary:#7561d1')
$html.Contains('共鸣广场')
```
...
- 梦境主题目前主要通过共享变量和末尾 CSS 覆盖完成，部分旧组件可能仍残留海绿色或旧阴影，需要依据截图逐页精修。
...
预期：工作区干净，当前分支为 `main`，本地与远端指向 `3e34aa4`，线上健康端点也报告该提交。若任一状态不同，先查明差异，不要直接覆盖。
