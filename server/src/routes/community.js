...
router.get('/search', (req, res) => {
  ...
  const moments = (db().moments || [])
    .filter(m => !m.deleted && m.type !== 'moment' && !m.zoneId && `${m.content || ''} ${m.topicLabel || ''} ${(m.topics || []).map(topic => topic.topicLabel || '').join(' ')}`.toLowerCase().includes(q))
    .sort((a, b) => b.createdAt - a.createdAt)
    .slice(0, 20)
    .map(m => {
      const author = findUserById(m.userId) || {};
      return { id: m.id, content: m.content, images: m.images || [], topicId: m.topicId || null, topicLabel: m.topicLabel || null, topics: m.topics || [], mood: m.mood || null, createdAt: m.createdAt, author: { id: m.userId, nickname: author.nickname || '用户', avatar: author.avatar || '' } };
    });
